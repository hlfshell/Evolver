...
module.exports = function(){
  ...
  evolver.prototype.addFunctionalNode = function(fnc, name, minParams, maxParams){
    name = name ? name : 'Functional Node ' + Math.random().toString(36).substring(7)
    var newNode = new FunctionalNode(fnc, name, 'functional', minParams, maxParams)
    newNode._next = this._next
    this.functionalSet.push(newNode)
  }

  evolver.prototype.addTerminalNode = function(fnc, name){
    name = name ? name : 'Terminal Node ' + Math.random().toString(36).substring(7)
    var newNode = new FunctionalNode(fnc, name, true)
    newNode._next = this._next
    this.terminalSet.push(newNode)
  }

  evolver.prototype.addConstantNode = function(constant){
    name = constant
    var newNode = new FunctionalNode(function(params, cb){
      cb(constant)
    }, name, true)
    newNode._next = this._next
    this.terminalSet.push(newNode)
  ...
  evolver.prototype.setCrossoverInputSelectionRate = function(rate){
    this.crossoverTerminalSelectionRate = rate
    this.crossoverFunctionSelectionRate = 1 - rate
  }

  evolver.prototype.setCrossoverFunctionSelectionRate = function(rate){
  ...
  evolver.prototype.rankPopulation = function(cb){
  ...
  evolver.prototype.calculatePopulationFitness = function(cb){
    var self = this
  ...
  evolver.prototype.calculateFitness = function(input, expectedOutput, actualOutput, cb){
    if(expectedOutput == 0){
  ...
  evolver.prototype.testOrganism = function(organism, cb){
    var self = this
    var fitnesses = []
  ...
  evolver.prototype.endCondition = function(cb){
  ...
  evolver.prototype.generateRandomOrganism = function(cb){
    var self = this
    self._next(function(){
      self.generateRandomNode(0, function(node){
        var organism = new Organism()
        organism.fnc = node
        organism._next = self._next
        cb(organism)
      })
    })
  }

  evolver.prototype.createOrganismFromString = function(string, cb){
    var self = this
    Organism.fromString(string, self.functionalSet.concat(self.terminalSet),
      function(organism){
        organsim._next = self._next
        cb(organism)
      })
  }

  //Generate a random node. For use in mutations and initial seeding
  evolver.prototype.generateRandomNode = function(currentDepth, cb){
  ...
  evolver.prototype.generateRandomPopulation = function(cb){
    var self = this
    self.population = []
  ...
  evolver.prototype.generateNextPopulation = function(cb){
    var self = this
  ...
  evolver.prototype.crossoverMate = function(parentA, parentB, cb){
    var potentialANodes = [],
      potentialBNodes = []
  ...
  evolver.prototype.mutateOrganism = function(node, cb, attempted){
    var self = this
    if(!attempted){
      attempted = []
    ...
    if(mutationType < replaceNode && attempted.indexOf('replace') == -1){
      ...
      self.mutationReplaceNode(node, function(err, result){
        ...
        if(err){
          self._next(self.mutateOrganism(node, cb, attempted))
        } else {
          self._next(cb(result))
        }
      })
  ...
  evolver.prototype.mutationAddNode = function(organism, cb, attempt){
    var self = this
          ...
          self.generateRandomNode(function(newNode){
            ...
            organism.setChildById(child, originalId, function(result){
              if(result){
                ...
                self._next(self.mutationAddNode(organism, cb, attempt))
              }
            })
          })
  ...
  evolver.prototype.mutationReplaceNode = function(organism, cb, attempt){
    var self = this
          ...
          organism.setChildById(newNode, chosenNode, function(result){
            if(false){
                ...
                if(attempt >= this.mutationAttemptLimit){
                  ...
                  self._next(self.mutationReplaceNode(organism, cb, ++attempt))
                }
            } else {
              cb(null, result)
            }
  ...
  evolver.prototype.mutationDeleteNode = function(organism, cb, attempt){
  ...
  evolver.prototype.execute = function(cb){
    var self = this
