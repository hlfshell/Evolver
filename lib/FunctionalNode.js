...
//( + y ( + x ( + ( - y y y ) x x ) ) ( + ( - ( - x x ) x ) ( - y x ) x ) ( + y x ) )
  FunctionalNode.fromString = function(source, nodeSet){
    var self = this
    source = source.split(' ')
    ...
    var parse = function(input){
      ...
      if(token == '('){
        token = input.shift()
        var functionalNode = getNodeByName(token)
        var nextItem = input[0]
        while(nextItem != ')'){
          if(nextItem == '('){
            functionalNode.params.push(parse(input))
          } else {
            token = input.shift()
            functionalNode.params.push(getNodeByName(token))
          }
          nextItem = input[0]
        }
