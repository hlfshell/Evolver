...
module.exports = function(){
  ...
  var pinnacle = function(){
    ...
    //If a result is within this accuracy, end early
  ...
  pinnacle.prototype.endCondition = function(cb){
    this.endOnAccuracy && this.endOnAccuracy <= this.population[0].fitness ?
      cb(true) : this.generation >= this.generationLimit ? cb(true) : cb(false)
  }
    ...
    self.generateNextPopulation(function(){
      setImmediate(function(){
        self.calculatePopulationFitness(function(){
          self.generation++
          console.log("Launching generation", self.generation, "Population is at", self.population.length, "Top ranked child at", self.population[0].fitness, self.population[0].toString())
