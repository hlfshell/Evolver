...
module.exports = function(){
  ...
  pinnacle.prototype.generateNextPopulation = function(cb){
    ...
    if(this.organismSurvival > 0){
      self.population.splice(0, this.organismSurvival).forEach(function(organism){
    ...
    if(self.crossoverRankLimit){
      self.population.splice(0, this.crossoverRankLimit).forEach(function(organism){
    ...
    while(newPopulation.length == this.population.length){
      ...
      var child = self.crossoverMate(parentA, parentB, function(newChild){
        ...
        if(mutateChoice < self.mutationRate){
          //Mutate!
          self.mutateOrganism(child, function(mutatedChild){
            child = mutatedChild
          })
        }
      })
  ...
  pinnacle.prototype.mutateOrganism = function(node, cb, attempted){
    ...
    if(mutationType < replaceNode && attempted.indexOf('replace') == -1){
      ...
      // })
      console.log('replace mutation chosen')
      attempted.push('replace')
      //Call add node
      setImmediate(self.mutationReplaceNode(node, function(err, result){
      ...
      //replace fails but add is an option
        ...
        console.log('add mutation chosen')
      attempted.push('add')
      //Call add node
      setImmediate(self.mutationAddNode(node, function(err, result){
        //If this has failed out after three attempts, call
        //mutateOrganism with attempted
        if(err){
          setImmediate(self.mutateOrganism(node, cb, attempted))
      ...
      setImmediate(function(){
        console.log('delete mutation chosen')
        attempted.push('delete')
        //Call add node
        setImmediate(self.mutationDeleteNode(node, function(err, result){
          //If this has failed out after three attempts, call
          //mutateOrganism with attempted
          if(err){
            setImmediate(self.mutateOrganism(node, cb, attempted))
  ...
  pinnacle.prototype.mutationAddNode = function(organism, cb, attempt){
    ...
    organism.getFunctionalNodeIds(function(targets){
      ...
      //Grab that node
      organism.getChildById(selectedNode, function(child){
