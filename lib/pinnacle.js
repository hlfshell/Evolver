...
module.exports = function(){
  ...
  //Generate a random node piece. For use in mutations and initial seeding
  pinnacle.prototype.generateRandomNode = function(){
      var self = this
      var nodeChoice = self.terminalSet.concat(self.functionalSet)

      if(nodeChoice.terminal){
        return nodeChoice
      } else {
        //Determine how many parameters we need to generate
        var paramCount
        if(node.maxParams){
          paramCount = Math.floor( (Math.random() * (node.maxParams - node.minParams)) + node.minParams)
        } else {
          paramCount = Math.floor( (Math.random() * ((5 + node.minParams) - node.minParams)) + node.minParams)
        }

        for(var i = 0; i < paramCount; i++){
          nodeChoice.addParam(self.generateRandomNode)
        }
  ...
  pinnacle.prototype.generateRandomPopulation = function(){
    var self = this
    self.population = []
  ...
  pinnacle.prototype.generateNextPopulation = function(){
    var self = this
  ...
  pinnacle.prototype.crossoverMate = function(parentA, parentB){
    var potentialANodes = [],
      potentialBNodes = []
  ...
  pinnacle.prototype.mutateNode = function(node){
    var targets = node.getFunctionalNodeIds()
    //Remove the parent node as a mutation prospect
    targets.splice(targets.indexOf(node.fnc.id), 1)
    targets.concat(node.getTerminalNodeIds())

    var chosenNode = targets[Math.floor(Math.random() * targets.length)]
    node.setChildById(self.generateRandomNode(), chosenNode)
  }
